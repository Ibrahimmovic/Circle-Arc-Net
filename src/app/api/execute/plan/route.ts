...
import { buildExecutionPlan } from "@/lib/cross-chain-execution";
import { compilePlanToJobs } from "@/lib/execution/compile-jobs";
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      topPositions =
        positions.data?.slice(0, 20).map((p) => ({
          name:
    ...
    const plan = buildExecutionPlan(analysis, topPositions);
    const jobs = compilePlanToJobs(plan.opportunities);
    plan.jobCount = jobs.length;
    return NextResponse.json({ plan, jobs, analysis });
  } catch (e) {
