...
export const EXECUTION_RAILS: ExecutionRail[] = [
  ...
  },
  {
    ...
    description: "Detect price gaps across chains; route via CCTP + swap (agent auto-fire).",
    status: "beta",
    rfb: "RFB #05",
  },
  {
  ...
  },
  {
    ...
    description: "Non-CCTP bridges (LiFi, etc.) when route not on Circle mesh.",
    status: "planned",
  },
];
...
function rebalanceToOpportunity(a: RebalanceAction): ExecutionOpportunity {
  return {
    ...
    priority: a.priority,
    executable: a.type === "bridge" && Boolean(a.toChain),
    executeHint: "Execute → CCTP panel with From/To chains prefilled.",
  };
}
...
export function detectArbitrageOpportunities(
  ...
  for (const [symbol, rows] of bySymbol) {
    ...
    out.push({
      ...
      executable: false,
      executeHint:
        "Agent will compose CCTP + swap steps; manual: use Execute panel per leg.",
    });
  }
